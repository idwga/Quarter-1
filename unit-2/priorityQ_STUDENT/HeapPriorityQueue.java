...
public class HeapPriorityQueue implements PriorityQueue
{
   ...
   private void reheapUp()
   {
      int Index = numItems;
      int IndexB = Index / 2;
      while (Index > 1 && higherPriority(items[Index], items[IndexB])) 
      {
         swap(items, Index, IndexB);
         Index = IndexB;
         IndexB = Index / 2;
      }
   ...
   private void reheapDown()
   {
      ...
      {
         int higheIndex = rightIndex <= numItems
         && higherPriority(items[rightIndex], items[leftIndex])? rightIndex: leftIndex;

         if (lowerPriority(items[Index], items[higheIndex]))
         {
            swap(items, Index, higheIndex);
            Index = higheIndex;
            leftIndex = Index * 2;
