...
public class animalGuesserShell
{
   ...
   private static void run() throws IOException 
   {
      ...
      while (playing) 
      {
          ...
          if (!questions[index].equals("0")) 
         {
            System.out.println(questions[index] + " (y/n)");
            ans = console.nextLine();
          }

          if (isYes(ans))
         {
              if (rightIndex < questions.length && !questions[rightIndex].equals("0")) 
              {
                  index = rightIndex;
              } 
              else 
         ...
         else if (isNo(ans) || questions[index].equals("0")) 
         {
              if (leftIndex < questions.length) 
              {
                  index = leftIndex;
              } 
              else 
              {
                  System.out.println("I lost. What is your animal?");
                  String obj = console.nextLine().toLowerCase();

                  System.out.println("What is a yes or no question that applies to your animal?");
                  String question = console.nextLine();

                  System.out.println("Does that question apply to your animal?  (y/n)");
                  String applies = console.nextLine();

                  String animalQuestion = "Is it" + aOrAn(obj) + "?";

                  String[] newQuestions = new String[questions.length * 4 + 2];
                  ...
                  if (isYes(applies)) 
                  {
                      newQuestions[index * 2] = question;
                      newQuestions[index * 4 + 1] = animalQuestion;
                  } 
                  else 
                  {
                      newQuestions[index * 2] = animalQuestion;
                      newQuestions[index * 4] = question;
                  }
